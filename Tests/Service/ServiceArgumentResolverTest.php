...
final class ServiceArgumentResolverTest extends TestCase
{
    ...
    public static function sensitiveParameterNameProvider(): iterable
    {
        ...
        yield 'dsn' => ['dsn'];
        yield 'databaseUrl' => ['databaseUrl'];
    }

    /**
     * A DSN commonly embeds credentials directly in the string (`mysql://user:pass@host/db`),
     * so redacting only "password"-shaped parameters would leak them through a `$dsn` argument.
     */
    public function testRedactsADsnEvenThoughItIsNotNamedLikeAPassword()
    {
        $resolved = $this->resolveAgainstSignature(['dsn'], [$this->literal('mysql://root:s3cr3t-pass@127.0.0.1:3306/app')]);
