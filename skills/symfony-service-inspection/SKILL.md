...
- `symfony-service-detail --id=<exact id>`: full detail for one service, `{id, class, public, synthetic, lazy, shared, abstract, autowire, autoconfigure, tags, calls, arguments, factory?}`. The id must be exact.
...
- **Tag not applied (listener/subscriber/extension silent):** `detail` the service and check `tags`. If the expected tag is absent, autoconfiguration did not fire: `autoconfigure: false` says so directly, otherwise the class does not implement the expected interface/attribute. Each tag entry carries its attributes (`event`, `priority`, `method`, ...); a listener bound to the wrong `event` or `priority` is a common cause.
